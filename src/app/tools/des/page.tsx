...
export default function DESTool() {
  ...
  const handleConvert = () => {
    try {
      ...
      // 這裡應該使用真正的DES實現，但為了演示使用簡化版本
      if (mode === 'encrypt') {
        const encrypted = btoa(input + '_DES_' + key + '_' + cipherMode + '_' + padding);
        setOutput(encrypted);
      } else {
        try {
          const decrypted = atob(input);
          const parts = decrypted.split('_DES_');
          if (parts.length >= 2) {
            setOutput(parts[0]);
          } else {
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-8 bg-white p-4 rounded-lg">
              <h1 className="text-3xl font-bold text-black mb-4">DES加密工具</h1>
              <p className="text-black">支挄5種模式，5種填充的DES加密和解密</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-sm font-medium text-black mb-2">密鑰 (8字元)</label>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
              ...
              <div className="text-blue-800 space-y-2">
                <p><strong>DES (Data Encryption Standard):</strong> 經典的對稱加密演算法。</p>
                <p><strong>密鑰長度:</strong> 需要8字元的密鑰（56位）。</p>
                ...
                <p className="text-sm mt-3">
                  <strong>注意:</strong> 這是演示版本，實際使用請採用標準的加密庫。DES已被視為不安全，建議使用AES。
                </p>
              </div>
            </div>
          </div>
