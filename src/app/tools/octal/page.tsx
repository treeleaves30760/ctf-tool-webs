  ...
  const handleConvert = () => {
    try {
      if (mode === 'encode') {
        ...
        const octalStr = input.replace(/\s+/g, separator === 'space' ? ' ' : '');
        const octalArray = separator === 'space' 
          ? octalStr.split(' ')
          : octalStr.match(/.{1,3}/g) || [];
        
        const bytes = octalArray.map(oct => {
          if (!/^[0-7]{1,3}$/.test(oct)) {
            throw new Error(`無效的八進制格式: ${oct}`);
          }
          const decimal = parseInt(oct, 8);
          if (decimal > 255) {
            throw new Error(`八進制值超出範圍: ${oct}`);
          }
          return decimal;
        });
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-8 bg-white p-4 rounded-lg">
              <h1 className="text-3xl font-bold text-black mb-4">八進制編碼工具</h1>
              <p className="text-black">將文字轉換為八進制編碼或將八進制編碼轉換為文字</p>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={mode === 'encode' ? '請輸入要編碼的文字...' : '請輸入八進制編碼...'}
                  />
                </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
              ...
              <div className="text-blue-800 space-y-2">
                <p><strong>八進制編碼:</strong> 將文字轉換為以8為基數的數字表示法。</p>
                <p><strong>編碼:</strong> 將普通文字轉換為八進制編碼。</p>
                <p><strong>解碼:</strong> 將八進制編碼轉換回原始文字。</p>
                <p><strong>分隔符:</strong> 可選擇使用空格分隔每個字節的八進制表示。</p>
                <p className="text-sm mt-3">
                  <strong>範例:</strong> "Hi" → "110 151" (空格分隔) 或 "110151" (無分隔)
                </p>
              </div>
            </div>
          </div>
